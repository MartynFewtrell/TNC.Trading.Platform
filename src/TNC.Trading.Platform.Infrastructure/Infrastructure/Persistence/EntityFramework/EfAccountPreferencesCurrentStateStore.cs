...
internal sealed class EfAccountPreferencesCurrentStateStore(PlatformDbContext dbContext, IAppliedBrokerEnvironmentContextResolver? contextResolver = null) : IAccountPreferencesCurrentStateStore
{
    ...
    {
        var brokerEnvironmentId = await ResolveBrokerEnvironmentIdAsync(change.BrokerEnvironment, cancellationToken).ConfigureAwait(false);
        ...
        entity ??= new AccountPreferencesCurrentStateEntity { AccountPreferencesCurrentStateId = Guid.NewGuid(), BrokerEnvironmentId = brokerEnvironmentId, PlatformEnvironment = change.PlatformEnvironment.ToString(), BrokerEnvironment = change.BrokerEnvironment.ToString(), ConcurrencyToken = [] };
    ...
    public async Task<AccountPreferencesDesiredStateCommitResult> FinalizeConfirmedSaveAsync(AccountPreferencesOperation operation, AccountPreferencesDesiredStateChange change, TrailingStopsPreferenceObservation observation, CancellationToken cancellationToken)
    {
        ...
        dbContext.TrailingStopsPreferenceObservations.Add(await ToEntityAsync(observation, cancellationToken).ConfigureAwait(false));
    ...
    private async Task<TrailingStopsPreferenceObservationEntity> ToEntityAsync(TrailingStopsPreferenceObservation observation, CancellationToken cancellationToken) => new() { TrailingStopsPreferenceObservationId = observation.Id, BrokerEnvironmentId = await ResolveBrokerEnvironmentIdAsync(observation.BrokerEnvironment, cancellationToken).ConfigureAwait(false), TrailingStopsEnabled = observation.TrailingStopsEnabled, ObservedAtUtc = observation.ObservedAtUtc, RecordedAtUtc = observation.RecordedAtUtc, PlatformEnvironment = observation.PlatformEnvironment.ToString(), BrokerEnvironment = observation.BrokerEnvironment.ToString(), AccountId = observation.AccountId, ObservationKind = observation.ObservationKind, Source = observation.Source, Actor = observation.Actor, CorrelationId = observation.CorrelationId };
