...
using ApplicationRetentionPolicy = TNC.Trading.Platform.Application.Services.OperationalRecordRetentionPolicy;
    ...
    ILogger<OperationalRecordRetentionProcessor> logger)
{
    ...
    {
        var plan = ApplicationRetentionPolicy.CreatePlan(
            timeProvider.GetUtcNow(),
            configuration["Retention:OperationalRecordsDays"]);
        var retainedSnapshotKind = plan.RetainedSnapshotKind.ToString();
        ...
        if (dbContext.Database.IsSqlServer())
        {
            ...
            var deletedEvents = await dbContext.OperationalEvents
                .Where(item => item.OccurredAtUtc < plan.Cutoff)
            ...
            var deletedRetainedIgLoginSnapshots = await dbContext.IgLoginSnapshots
                .Where(item => item.SnapshotKind == retainedSnapshotKind
