...
The platform needs a safe and explicit foundation for choosing the intended IG environment, managing operator-editable configuration in `MS SQL Server`, defining a configurable trading schedule, and establishing and maintaining an authenticated IG demo session only when the platform is within permitted trading periods. The foundation must preserve environment separation, protect secrets, expose degraded and recovered session states, support an initial aggressive retry sequence followed by continued periodic retry and manual restart while the configured trading schedule is active, allow supported configuration updates through the Blazor UI, and provide enough operational visibility to keep the platform usable while auth-dependent actions remain blocked.
...
- Operator-managed platform, broker, trading-schedule, retry, and notification configuration is stored in SQL Server and loaded by the API at startup.
...
- For local desktop validation, notification delivery will be exercised through Mailpit configured as an Aspire integration while the production-oriented notification provider abstraction continues to target Azure Communication Services Email.
...
- Trading schedule configuration includes a configurable daily start-of-day and end-of-day, permitted trading days, configurable weekend treatment, and configurable bank-holiday exclusions; automatic holiday feeds or exchange-specific calendar integrations are not included here.
...
- The implementation must satisfy `FR1`-`FR22`, `NF1`-`NF5`, `SR1`-`SR5`, `DR1`, `IR1`-`IR5`, `TR1`-`TR13`, and `OR1`-`OR8` from `requirements.md`.
...
- The platform must maintain and retry the `IG` demo connection only while the configured trading schedule is active and must not treat out-of-schedule disconnection as a degraded auth failure.
...
1. **SQL Server-backed configuration management and startup validation** to require an explicit broker-environment choice from the configuration database, capture trading-schedule configuration, enforce Test-platform live restrictions before any live action is attempted, and manage separate environment-specific settings plus protected secret values or secret references.
2. **A trading-schedule-aware session-supervision background workflow** to establish the IG demo session only while the configured trading schedule is active, detect invalid or expired sessions, drive automatic retry cycles with the required initial exponential backoff followed by periodic retry while the trading schedule remains active, stop maintaining the connection outside the permitted schedule, expose degraded-state status, and allow an operator-triggered manual retry that resets the retry budget.
3. **A dedicated Blazor Web App with server interactivity** to expose the selected environment, configuration state, trading-schedule state, auth/session state, retry progress, blocked reasons, manual-retry availability, configuration update flows, and recorded auth-related events without exposing secrets.

The API will host the background session supervisor, configuration read/write endpoints, protected credential-update flows, and the persistence logic for SQL-backed configuration plus environment-scoped operational records. A new .NET 10 Blazor Web App with server interactivity will consume those APIs and remain available during degraded auth state so the operator can review status, update supported configuration, and trigger manual retry when appropriate. The selected broker environment and configured trading schedule will be loaded from the configuration database at startup, displayed by the UI as active context, and updated only through validated operator flows rather than direct file edits. Changes to startup-fixed settings such as platform environment and broker environment will be persisted for the next startup and will not introduce runtime environment switching in this work package. The session supervisor will treat out-of-schedule connectivity as intentionally inactive rather than degraded, will establish or resume the broker session only while the configured trading schedule is active, and will stop maintaining the connection after the configured trading period ends or on configured non-trading days such as excluded weekends or configured bank holidays. Server interactivity keeps the initial implementation simpler than a separate WebAssembly client, allows straightforward secure server-side access patterns for the UI application, and still provides the responsive operator experience required by the work package. Durable configuration, current state, and records should be stored in SQL Server to align with repository defaults for relational persistence and to support the 90-day retention requirement. Authentication and notification interactions with external systems should be isolated behind focused infrastructure components, with email dispatch routed through a notification provider abstraction backed by Azure Communication Services Email, so configuration updates, trading-schedule gating, retry logic, redaction, retry-phase transitions, and state transitions remain testable.
...
The work package adds a broker-auth and configuration-management control plane to the existing API application and a dedicated Blazor Web App with server interactivity. SQL-backed configuration selects the platform environment, broker environment, trading-schedule settings, retry settings, and notification settings at startup. A startup validator blocks unsupported combinations, especially IG live in the Test platform environment, and the application does not proceed without a configured explicit broker-environment value loaded from SQL Server. A hosted session supervisor then establishes the IG demo session while the configured trading schedule is active, updates a persisted session-state projection, records state-transition events, and dispatches notifications when required. The supervisor operates in two retry phases while degraded during active trading periods: an initial exponential retry sequence and a continued periodic retry mode after that sequence is exhausted. Outside the configured trading schedule, including configured non-trading weekends and configured bank holidays, the supervisor stops maintaining the broker session, suppresses auth retry activity, and marks the state as intentionally out of schedule rather than degraded. The Blazor UI reads the current state and active configuration from the API, keeps degraded-state views available, presents the active broker environment and trading-schedule state as runtime context, offers controlled configuration editing, shows the current retry phase and next scheduled retry time when applicable, and exposes the manual-retry action only after the initial retry limit has been reached, while the trading schedule is active, and when a retry is not already running.
...
- **Components**:
  ...
  - `ConfigurationStore` for SQL-backed platform, broker, trading-schedule, retry, and notification configuration plus change auditing
  ...
  - `TradingScheduleGate` for deciding whether broker connectivity should be active based on configured start-of-day, end-of-day, permitted trading days, weekend treatment, and bank-holiday exclusions
  - `IgSessionSupervisor` background service for startup auth, trading-schedule transitions, expiry detection, retry cycles, scheduled disconnect, and recovery
  - `AuthStateStore` for current environment/session/retry/trading-schedule status projections
  - `OperationalEventLedger` for append-only auth, retry, trading-schedule, blocked-live, and notification records
...
- **Data flows**:
  - Startup reads platform, broker, and trading-schedule settings from SQL Server, validates the environment combination, and refuses to start without an explicit broker-environment choice supplied through the configuration store
  ...
  - When startup occurs during the configured trading schedule, the session supervisor authenticates to IG demo, records the outcome, and updates the current session projection; when startup occurs outside the configured trading schedule, it records an out-of-schedule state and defers broker connection activity
  - On auth/session failure during the configured trading schedule, the supervisor marks the platform degraded, records the state transition, emits a failure notification once per retry cycle, and schedules the next retry attempt
  - When the initial automatic retry sequence is exhausted during the configured trading schedule, the supervisor records the transition into periodic retry mode, emits a retry-limit notification, and continues scheduling reconnect attempts on the configured periodic delay until recovery, manual retry, or the end of the active trading period
  - At the configured end of day, on configured non-trading weekends, or on configured bank holidays, the supervisor stops maintaining the broker session, records the transition to out-of-schedule state, and suppresses further auth retry until the next active trading period
  - The Blazor UI queries status endpoints to show environment, trading-schedule state, session state, retry attempt number, next scheduled retry time, blocked reasons, and manual-retry availability
  - When manual retry is triggered from the UI after the initial retry limit has been reached and while the configured trading schedule is active, the supervisor starts a new attempt immediately without waiting for the next periodic retry and resets the automatic retry budget if the manual attempt fails
...
- **Dependencies**:
  ...
  - Mailpit configured through Aspire for local notification validation
...
| FR21 | Support configurable trading schedule with start-of-day, end-of-day, permitted trading days, weekend treatment, and bank-holiday exclusions | Add trading-schedule configuration fields, validation, and status exposure through the API and Blazor Web App | API, UI, and configuration-persistence tests that verify trading-schedule review and update behavior |
| FR22 | Maintain the IG demo connection only during the configured trading schedule | Add trading-schedule gating in the session supervisor so auth, retry, and manual retry activity operate only during active permitted periods and out-of-schedule state is treated as intentional | Integration and functional tests for in-schedule connection behavior, out-of-schedule disconnect behavior, and retry suppression |
| NF1 | Support sustained safe operation under session expiry and transient auth failure during the configured trading schedule | Use supervised recovery, bounded initial retries, continued periodic retry during active trading periods, preserved environment context, and out-of-schedule gating instead of failing the whole app | Integration tests for expiry, invalid-session recovery, continued degraded periodic retry state, and out-of-schedule suppression |
| NF2 | Expose enough environment, configuration, trading-schedule, and session-state information for operator understanding | Provide current-environment, active non-secret configuration, configured trading-schedule, current in-schedule or out-of-schedule state, session-state, retry-progress, retry phase, blocked-reason, and manual-retry availability read models plus durable records | API and persistence tests that verify each required signal is available during runtime and after events |
...
| SR5 | Include useful notification context without exposing secrets | Build notifications from redacted state-transition models that include event type, timestamp, environment context, concise summary, and any event-specific non-secret fields such as retry-limit details | Notification content tests for all supported event types |
| DR1 | Retain environment selection, configuration-change, and auth/session records for 90 days | Store configuration-audit records, operational events including trading-schedule transitions, and notification records with environment context and apply a 90-day retention policy | Persistence tests and retention-policy verification |
...
| TR13 | Verify trading-schedule configuration and connection-window behavior | Add tests for trading-schedule configuration updates, in-schedule connection activity, out-of-schedule disconnect behavior, retry suppression, and visible trading-schedule state | Requirement-driven test cases mapped to `FR21`, `FR22`, `NF1`, and `NF2` |
| OR1 | Surface current environment, trading-schedule state, and auth/session state during runtime | Provide the platform-status read model in the API and render degraded-state indicators plus trading-schedule state in the Blazor operator UI | API contract tests and operator-surface functional tests |
| OR2 | Record notable auth/session state transitions for later review | Persist append-only operational events for auth success, failure, initial retries, transition into periodic retry, periodic retries, manual retry, exhaustion of initial retry sequence, expiry, recovery, and trading-schedule transitions | Persistence and integration tests over event history |
...
| OR4 | Notify the project owner on supported auth/session, retry-limit transition into periodic retry, recovery, blocked-live, and new failure-cycle events | Use the notification dispatcher with event-specific policies for failure, retry-limit reached/transition into periodic retry after initial retry exhaustion, recovery, blocked live attempts, and per-cycle failure notifications through the notification provider abstraction | Notification pipeline tests and integration tests for each event type |
...
| OR6 | Show retry progress details while automatic or periodic retry is active during the configured trading schedule | Publish retry attempt number, retry phase, and next scheduled retry time in the status API and refresh the Blazor Web App as retry state changes during active trading periods | Functional and integration tests for retry-progress transitions |
| OR7 | Let the operator review and update supported configuration from the Blazor UI without exposing stored IG secrets | Provide configuration read models, audited update commands, write-only secret update controls, and trading-schedule editing in the Blazor Web App | Functional, API, and persistence tests for configuration review, update, and secret-safe rendering |
| OR8 | Make trading-schedule-based connection state visible to the operator | Publish configured trading-schedule values and current in-schedule or out-of-schedule state in the status API and render them in the Blazor Web App | Functional and integration tests for visible trading-schedule state |
...
| ---- | -------- | ------- | ----- |
| UI | `/status` page in the Blazor Web App | Shows platform environment, configured broker environment, configured trading schedule, current in-schedule or out-of-schedule state, current auth/session state, degraded banner, retry progress, current retry phase, live-option availability, and blocked reasons | Primary operator view for degraded-state and trading-schedule visibility |
| UI | `/configuration` page in the Blazor Web App | Shows active non-secret configuration including trading-schedule settings, write-only credential update fields, validation messages, last configuration-change details, and indicators for startup-fixed changes that apply on next restart | Primary operator view for SQL-backed configuration management |
| UI | Manual retry action in the Blazor Web App | Button is enabled only after retry exhaustion while the configured trading schedule is active and issues the backend manual-retry command with no confirmation step | Supports `FR15`, `FR16`, and `FR22` |
| REST | `GET /api/platform/status` | Response includes platform environment, broker environment, configured trading schedule, current in-schedule or out-of-schedule state, live-option availability, auth/session state, degraded flag, blocked reasons, retry state, manual-retry availability, and last transition timestamp | Primary operator read model for `FR2`, `FR13`, `FR18`, `FR22`, and `OR1` |
| REST | `GET /api/platform/configuration` | Response includes active non-secret configuration values, trading-schedule settings, secret placeholders, and last-updated metadata | Supports `FR20`, `FR21`, and `OR7` without returning stored secrets |
| REST | `PUT /api/platform/configuration` | Request contains supported configuration changes including trading-schedule values and optional write-only secret fields; response returns redacted updated configuration plus whether a restart is required for startup-fixed changes | Persists configuration to SQL Server and records an audit entry |
...
| Internal event | `PlatformConfigurationUpdated` | `RetrySettingsChanged`, `NotificationSettingsChanged`, `TradingScheduleChanged`, `BrokerEnvironmentChanged`, `CredentialsUpdated` | Internal application event used to persist configuration-audit entries and support observability |
| Internal event | `AuthStateTransitionRecorded` | `FailureDetected`, `RetryScheduled`, `RetryExhausted`, `Recovered`, `TradingScheduleStarted`, `TradingScheduleEnded`, `BlockedLiveAttempt` | Internal application event used to persist ledger entries and dispatch notifications |
...
| -------------- | ------ | ----------- | ----- |
| PlatformConfiguration | `ConfigurationId`, `PlatformEnvironment`, `BrokerEnvironment`, `TradingHoursStart`, `TradingHoursEnd`, `TradingDays`, `WeekendBehavior`, `BankHolidayExclusions`, `NotificationSettings`, `RetrySettings`, `UpdatedAtUtc`, `UpdatedBy`, `Version`, `RestartRequired` | Active row is required before auth supervision starts; start-of-day must be earlier than end-of-day; configured trading days must be valid; optimistic concurrency should protect updates | SQL-backed source of truth for operator-managed non-secret configuration |
...
| TradingSchedule | `TradingHoursStart`, `TradingHoursEnd`, `TradingDays`, `WeekendBehavior`, `BankHolidayExclusions`, `TimeZone` | Start-of-day must be earlier than end-of-day for a single-day schedule; bank-holiday values must be valid calendar dates | Defines when broker connectivity may be active |
| IgSessionState | `BrokerEnvironment`, `TradingScheduleStatus`, `SessionStatus`, `EstablishedAtUtc`, `LastValidatedAtUtc`, `ExpiresAtUtc`, `IsDegraded`, `BlockedReason`, `CurrentRetryCycleId` | Only demo sessions may reach `Active` in this work package; out-of-schedule state is distinct from degraded | Projection of the current broker-auth state |
...
| OperationalEvent | `EventId`, `OccurredAtUtc`, `EventType`, `PlatformEnvironment`, `BrokerEnvironment`, `Severity`, `Summary`, `DetailsJson`, `CorrelationId` | `DetailsJson` must contain only redacted payloads | Append-only ledger for auth, retry, trading-schedule, blocked-live, and notification events |
...
| ---- | ------ | ------------- | ----- |
| 1 | Add SQL-backed platform, broker, trading-schedule, retry, notification, and protected credential configuration models plus startup validation | `src/TNC.Trading.Platform.Api/Configuration/*`, `Program.cs` | Validation must fail fast for missing environment selection, invalid trading-schedule values, and unsupported Test+Live combinations; configuration is sourced from SQL Server in this work package |
...
| 5 | Implement the hosted session supervisor, trading-schedule gate, and retry scheduler | `src/TNC.Trading.Platform.Api/Infrastructure/Auth/*` | Supervisor owns degraded startup during active trading periods, scheduled connect and disconnect, expiry detection, initial exponential retry, periodic retry after initial exhaustion, manual retry reset, and recovery transitions |
...
| 7 | Add the Blazor Web App for status, configuration editing, trading-schedule visibility, degraded-state messaging, live-option visibility, and manual retry | `src/TNC.Trading.Platform.Web/*` | Use server interactivity and keep the UI usable while auth-dependent actions remain visibly blocked |
...
| 9 | Extend AppHost orchestration for the Blazor UI, SQL Server, Mailpit, and local configuration wiring | `src/TNC.Trading.Platform.AppHost/AppHost.cs` | Keep local developer flow under Aspire, include Mailpit for local notification validation, and reuse service-default observability |
...
| Trading schedule is configured with an invalid or nonsensical window, trading-day set, or bank-holiday entry | Reject the configuration, keep the last known valid trading-schedule values, and surface the validation error without altering active behavior | Validation log, configuration-audit record, and UI validation state |
...
| Initial IG demo authentication fails during the configured trading schedule | Start the platform in degraded state, record failure, notify once for the cycle, and schedule automatic retry attempt 1 | Failure event, retry-scheduled event, degraded-state metric |
| Startup occurs outside the configured trading schedule | Do not establish or maintain an IG session, record the out-of-schedule state, and expose that the platform is waiting for the next active trading period | Trading-schedule state event, status-read-model update, informational log |
...
| Configured end of day is reached, a configured non-trading weekend begins, or a configured bank holiday becomes active while a session is active | Stop maintaining the IG session, record the transition to out-of-schedule state, and suppress further retry until the trading schedule resumes | Trading-schedule-ended event, status-read-model update, informational log |
...
| Manual retry requested while retry is already running, while retry exhaustion has not occurred, or while the trading schedule is inactive | Reject the command with an explicit reason, keep the UI available, and show the blocked reason without altering current state | Command warning log, API response with blocked reason, and UI feedback state |
...
| `Broker:Ig:TradingHours:StartOfDay` | Start of the configured daily trading window | None; must be supplied | SQL Server configuration store |
| `Broker:Ig:TradingHours:EndOfDay` | End of the configured daily trading window | None; must be supplied | SQL Server configuration store |
| `Broker:Ig:TradingSchedule:TradingDays` | Permitted trading days within the weekly schedule | Monday-Friday | SQL Server configuration store |
| `Broker:Ig:TradingSchedule:WeekendBehavior` | Whether Saturday and Sunday are non-trading or explicitly enabled | Exclude weekends | SQL Server configuration store |
| `Broker:Ig:TradingSchedule:BankHolidays` | Configurable list of excluded bank-holiday dates | Empty | SQL Server configuration store |
...
- **Threat model notes**:
  - Missing or invalid credentials during active trading periods lead to degraded startup rather than a false healthy state.
  ...
  - Trading-schedule misconfiguration is mitigated by validation, visible status exposure, and explicit out-of-schedule state handling.
  ...
  - Manual retry abuse is limited by state guards that allow the command only when retry exhaustion has been reached, the trading schedule is active, and no retry is already running.

## 7. Observability

| Signal | What | Where | Notes |
| ------ | ---- | ----- | ----- |
| Logs | Startup configuration validation, configuration updates, trading-schedule transitions, auth success/failure, session expiry detection, retry scheduling, retry exhaustion, manual retry commands, blocked live attempts, notification dispatch outcomes | Structured application logs via existing OpenTelemetry logging | Must include platform and broker environment context and must exclude secrets |
| Metrics | Current degraded-state flag, current trading-schedule-active flag, auth retry attempts, current retry phase, periodic retry count, retry-limit reached count, auth recoveries, blocked live-attempt count, configuration-update count, notification dispatch failures | OpenTelemetry metrics exported through the existing service-defaults pipeline | Supports dashboards and alerts for sustained degraded state, out-of-schedule state, and configuration churn |
| Traces | Configuration read/write requests, trading-schedule gate decisions, outbound IG authentication calls, session-validation checks, Blazor Web App requests to the API, manual-retry command execution, notification dispatch operations, persistence writes for configuration changes and state transitions | OpenTelemetry tracing in the API and UI services | Health endpoints remain excluded per service defaults |

## 8. Testing Strategy

| Test type | Coverage | Location | Notes |
| --------- | -------- | -------- | ----- |
| Unit | Startup validators, configuration validators, trading-schedule validation and gating, retry policy timing, retry-cycle state transitions, redaction helpers, live-blocking rules, protected-credential handling, and notification mappers | `test/TNC.Trading.Platform.Api/TNC.Trading.Platform.Api.UnitTests` | Add fast deterministic tests for edge cases, secret scrubbing, trading-schedule rules, and configuration update rules |
| Integration | AppHost-based tests for configuration load/update behavior, trading-schedule gating, startup auth success/failure, degraded startup, transition from initial retry to periodic retry, manual retry, persistence, and notification dispatch behavior | `test/TNC.Trading.Platform.Api/TNC.Trading.Platform.Api.IntegrationTests` | Reuse the existing Aspire testing pattern already present in the repository |
| Functional | Requirement-driven UI and API tests for `FR1`-`FR22`, organized under the work-package folder | `test/TNC.Trading.Platform.Web/TNC.Trading.Platform.Web.FunctionalTests/002-environment-and-auth-foundation` | UI-driven functional tests should follow Playwright guidance and cover configuration management, trading-schedule visibility, and degraded retry visibility |
| E2E | Cross-project validation of the Blazor Web App and API working together under Aspire | `test/TNC.Trading.Platform.Web/TNC.Trading.Platform.Web.E2ETests` | Add only the minimal flows needed for configuration editing, trading-schedule enforcement, degraded-state visibility, periodic retry visibility, manual retry, and local notification verification through Mailpit |

## 9. Rollout Plan

| Phase | Action | Success criteria | Rollback |
| ----- | ------ | ---------------- | -------- |
| 1 | Deploy the API and Blazor Web App with SQL-backed configuration management, protected IG credential storage, configurable trading schedule, explicit Test-platform and IG demo configuration, SQL persistence, and auth supervision enabled | Platform starts with a working operator UI, visible environment/auth status, visible trading-schedule status, editable supported configuration, and either an active IG demo session during permitted trading periods, an intentional out-of-schedule state, or a visible degraded state with retry progress that continues into periodic retry after the initial retry sequence is exhausted | Disable the new UI project and auth supervisor registration and revert configuration/database changes |
