...
- Expose the theme switcher from the top-level header so the operator can access it consistently from signed-in surfaces without duplicating the control in page content.
...
| FR6 | The refreshed operator UI must provide a basic light and dark theme switching capability. | An explicit theme choice can improve usability while staying within the package's UI refresh scope. | The delivered UI allows the operator to switch between a light theme and a dark theme across the shared shell and refreshed pages without breaking the intended operator workflows. | This package should provide basic theme switching only, not a broader multi-theme platform. The theme switcher should be available from the top-level header for signed-in operators, and theme changes should apply immediately when selected. |
