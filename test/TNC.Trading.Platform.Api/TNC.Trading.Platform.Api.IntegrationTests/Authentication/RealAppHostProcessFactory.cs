...
internal static class RealAppHostProcessFactory
{
    ...
    public static AppHostProcessHandle StartAppHostProcess()
    {
        ...
        startInfo.Environment["Authentication__Test__EnableInteractiveSignIn"] = bool.FalseString;

        var process = Process.Start(startInfo)
            ?? throw new InvalidOperationException("Failed to start the AppHost process for the API authentication integration tests.");

        return new AppHostProcessHandle(process, existingPlatformProcessIds, existingListeningPorts);
    }

    public static Task<Uri> GetApiBaseUriAsync(AppHostProcessHandle appHostProcess)
    {
    ...
    private static string GetRepositoryRoot()
    {
        ...
        throw new InvalidOperationException("Could not locate the repository root for the API authentication integration tests.");
    }
}
